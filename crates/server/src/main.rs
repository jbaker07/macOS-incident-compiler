...
use bundle_exchange::{
    ...
    mark_as_imported, validate_bundle, ExportBundleRequest,
    ExportBundleResponse, ImportBundleResponse,
    RecomputeRequest, RecomputeResult,
};
use capture_control::{
    CaptureProfile, ProfileConfig, ThrottleController, ThrottleDecision,
};
use db::Database;
...
async fn get_signal_explanation(
    ...
    match state.db.get_signal_explanation(&id) {
        ...
        Ok(None) => (
            StatusCode::NOT_FOUND,
            ApiResponse::err(&format!("Explanation not found for signal '{}'. Signal was created before explainability was enabled, or locald did not generate an explanation.", id)),
        ),
        Err(e) => (
            StatusCode::INTERNAL_SERVER_ERROR,
            ApiResponse::err(&format!("Database error: {}", e)),
