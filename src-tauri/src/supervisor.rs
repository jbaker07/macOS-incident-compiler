...
//!
//! This module provides a unified supervisor for the entire EDR stack on macOS:
//! - capture_macos_rotating.exe: Windows event telemetry capture
...
impl Supervisor {
    ...
    pub async fn new() -> Result<Self, String> {
        ...
        // Determine telemetry root: prefer %LOCALAPPDATA%\macos-incident-compiler\telemetry
...
fn get_telemetry_root() -> PathBuf {
    ...
    // 2. Use %LOCALAPPDATA%\macos-incident-compiler\telemetry
    if let Some(local_app_data) = dirs::data_local_dir() {
        return local_app_data
            .join("macos-incident-compiler")
            .join("telemetry");
    }

    // 3. Fallback to C:\ProgramData\edr
    PathBuf::from("/tmp/edr")
}
